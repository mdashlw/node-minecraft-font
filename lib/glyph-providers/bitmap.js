...
export async function create({ height = 8, ascent, chars, file }) {
  ...
  const texture = await fs.readFile(`./assets/textures/${file.slice(file.indexOf(":") + 1)}`);
  ...
  for (let rowIndex = 0; rowIndex < chars.length; rowIndex += 1) {
    ...
    for (let index = 0; index < codepoints.length; index += 1) {
      ...
      glyphs[char] = {
        ...
        render(context, x, y, color) {
          ...
          context.drawImage(bufferCanvas, 0, 0, width, height, x, y, width, height);
        },
      };
    }
  }
